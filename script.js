...
function setActivePage(pageName, { haptic = false } = {}) {
  ...
  if (page === "admin-mailings") renderAdminMailings();
...
function pickupAddressValue() {
  return String(appSettings.pickupAddress || "").trim();
}

function pickupAddressLabel(value = pickupAddressValue()) {
  return String(value || "").trim() || DEFAULT_PICKUP_ADDRESS_LABEL;
}

function updatePickupAddressPreview(value = pickupAddressValue()) {
  const preview = document.querySelector("#adminPickupAddressPreview");
  if (preview) preview.textContent = pickupAddressLabel(value);
}

function applyPickupAddressToCheckout() {
  const pickupText = document.querySelector("#pickupAddressText");
  if (pickupText) pickupText.textContent = pickupAddressLabel();
  updatePickupAddressPreview();
}
...
function updateSummary() {
  ...
  if (!cart.length) {
    ...
    }).join("") + `<div class="summary-line summary-line--delivery"><div><span>${deliveryType === "pickup" ? "Самовывоз" : "Доставка"}</span><small>${deliveryType === "pickup" ? escapeHtml(pickupAddressLabel()) : (totals.delivery ? "Стандартный тариф" : "Бесплатная доставка")}</small></div><strong>${totals.delivery ? money(totals.delivery) : "0 ₽"}</strong></div>`;
  }
...
function updateDeliveryFields() {
  ...
  const pickupPanel = document.querySelector("#pickupAddressPanel");
...
async function submitOrder(event) {
  ...
  const payload = {
    ...
    delivery: {
      ...
      address: document.querySelector('input[name="delivery_type"]:checked').value === "delivery" ? document.querySelector("#deliveryAddress").value.trim() : pickupAddressValue(),
...
const demoOrders = [
  {id: 1024, status: "new", label: "Новый", customer_name: "Анна", total: 1890, items: "Букет «Розовый рассвет»"},
  {id: 1025, status: "new", label: "Новый", customer_name: "Мария", total: 2490, items: "Букет + открытка"},
  {id: 1026, status: "work", label: "В работе", customer_name: "Иван", total: 3200, items: "Большой букет"},
  {id: 1027, status: "ready", label: "Готов", customer_name: "Елена", total: 1500, items: "Игрушка + открытка"}
];
...
function openAdminOrder(order) {
  const overlay = document.querySelector("#adminOrderOverlay");
  const details = document.querySelector("#adminOrderDetails");
  if (!overlay || !details) return;

  const itemImage = order.image || order.photo || order.product_image || "images/rozovyi-rassvet.webp";
  const items = order.items || "Товар не указан";

  details.innerHTML = `
    <div class="admin-order-details">
      <div class="admin-order-details__top">
        <div>
          <span>Заказ #${order.id}</span>
          <h2>${escapeHtml(order.label || order.status || "Новый")}</h2>
        </div>
      </div>

      <section class="admin-order-product">
        <img src="${escapeHtml(itemImage)}" alt="${escapeHtml(items)}">
        <div>
          <h3>${escapeHtml(items)}</h3>
          <p><b>Размер:</b> ${escapeHtml(order.size || "Не указан")}</p>
          <p><b>Количество:</b> ${order.quantity || 1}</p>
        </div>
      </section>

      <section class="admin-order-info">
        <p><b>Имя заказчика:</b> ${escapeHtml(order.customer_name || "Не указано")}</p>
        <p><b>Имя получателя:</b> ${escapeHtml(order.recipient_name || "Не указано")}</p>
        <p><b>Телефон заказчика:</b> ${escapeHtml(order.customer_phone || "Не указан")}</p>
        <p><b>Телефон получателя:</b> ${escapeHtml(order.recipient_phone || "Не указан")}</p>
        <p><b>Адрес доставки:</b> ${escapeHtml(order.address || "Не указан")}</p>
        <p class="admin-order-total"><b>Сумма заказа:</b> ${money(order.total || 0)}</p>
      </section>
    </div>
  `;
  overlay.hidden = false;
...
function adminCustomersEmptyStateMarkup(rawQuery, totalClients) {
  const hasQuery = Boolean(String(rawQuery || "").trim());
  const title = hasQuery ? "Клиент не найден" : "Клиентов нет";
  const text = hasQuery
    ? `По номеру «${escapeHtml(rawQuery.trim())}» ничего не найдено. Проверьте цифры или очистите поиск.`
    : "После первого заказа карточки клиентов появятся здесь автоматически.";
  const action = hasQuery
    ? '<button class="admin-customers-empty__button" type="button" data-admin-customers-clear>Очистить поиск</button>'
    : "";
  const meta = totalClients > 0
    ? `<span>${totalClients} ${pluralizeAdminCustomers(totalClients)} в базе</span>`
    : "";

  return `
    <article class="admin-customers-empty card-surface">
      <div class="admin-customers-empty__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24"><circle cx="10" cy="8" r="4"/><path d="M4 20a6 6 0 0 1 12 0"/><path d="m17 17 3 3"/><circle cx="18" cy="14" r="3"/></svg>
      </div>
      <div class="admin-customers-empty__copy">
        ${meta}
        <strong>${title}</strong>
        <p>${text}</p>
      </div>
...
function pluralizeAdminCustomers(count) {
  const mod10 = count % 10;
  const mod100 = count % 100;
...
function renderAdminCustomers() {
  const list = document.querySelector("#adminCustomersList");
  if (!list) return;

  const searchInput = document.querySelector("#adminCustomersSearch");
  const rawQuery = searchInput?.value.trim() || "";
  const hasSearch = Boolean(rawQuery);
  const query = normalizeAdminCustomerPhone(rawQuery);
  const map = new Map();

  demoOrders.forEach(order => {
    const key = order.customer_name || "Без имени";
    const item = map.get(key) || {
      name: key,
      phone: order.customer_phone || order.phone || "Не указан",
      count: 0,
      total: 0
    };
    item.count++;
  ...
  const allClients = [...map.values()];
  const clients = allClients.filter(client => {
    if (!hasSearch) return true;
    if (!query) return false;
    return normalizeAdminCustomerPhone(client.phone).includes(query);
  });

  if (!clients.length) {
    list.innerHTML = adminCustomersEmptyStateMarkup(rawQuery, allClients.length);
    return;
  }

  list.innerHTML = clients.map(client => `
    <article class="admin-customer-card card-surface">
      <h3>${escapeHtml(client.name)}</h3>
      <p><b>Телефон:</b> ${escapeHtml(client.phone)}</p>
      <p><b>Заказов:</b> ${client.count}</p>
      <p><b>Сумма покупок:</b> ${money(client.total)}</p>
    </article>
  `).join("");
}

const adminMailingsStorageKey = "bloombox-admin-mailings";
const adminMailingMediaPersistLimit = 3 * 1024 * 1024;
let adminMailingFilter = "all";
let adminMailingSearch = "";
let adminMailingSelectedMedia = null;
let adminMailingSelectedMediaUrl = "";

const demoAdminMailings = [
  {
    id: 7101,
    title: "Скидка 10% на новые букеты",
    message: "Напомнить клиентам о новой коллекции и промокоде на первый заказ недели.",
    audience: "Все клиенты",
    channel: "Telegram",
    status: "sent",
    statusLabel: "Отправлена",
    sentCount: 42,
    date: "08.08.2026 в 15:30"
  },
  {
    id: 7102,
    title: "Акция на выходные",
    message: "Подготовить короткое сообщение про готовые букеты к самовывозу.",
    audience: "Клиенты с заказами",
    channel: "Telegram",
    status: "draft",
  ...
  {
    id: 7103,
    title: "Напоминание о предзаказе",
    message: "Предложить оформить доставку заранее перед праздничной датой.",
    audience: "Постоянные клиенты",
    channel: "Telegram",
    status: "scheduled",
    statusLabel: "Запланирована",
    sentCount: 18,
    date: "12.08.2026 в 10:00"
  }
];

function readAdminMailings() {
  try {
    const saved = JSON.parse(localStorage.getItem(adminMailingsStorageKey) || "[]");
...
function saveAdminMailings(items) {
  localStorage.setItem(adminMailingsStorageKey, JSON.stringify(items || []));
}

function getAdminMailings() {
  const saved = readAdminMailings();
  const savedIds = new Set(saved.map((item) => Number(item.id)));
  return [
    ...saved,
    ...demoAdminMailings.filter((item) => !savedIds.has(Number(item.id)))
  ];
}

function adminMailingStatusClass(status) {
  if (status === "sent") return "is-sent";
  if (status === "scheduled") return "is-scheduled";
  return "is-draft";
}

function formatAdminMailingFileSize(bytes) {
...
function getAdminMailingMediaKind(file = {}) {
  const type = String(file.type || "").toLowerCase();
  const name = String(file.name || "").toLowerCase();
  if (type.startsWith("image/") || /\.(png|jpe?g|webp|gif|heic)$/i.test(name)) return "image";
  if (type.startsWith("video/") || /\.(mp4|mov|webm|m4v|avi)$/i.test(name)) return "video";
  return "";
}

function adminMailingAttachmentLabel(attachment = {}) {
  if (!attachment?.kind) return "";
  return attachment.kind === "video" ? "Видео" : "Фото";
}

function adminMailingAttachmentMarkup(attachment) {
  if (!attachment) return "";

  const label = adminMailingAttachmentLabel(attachment);
  const name = escapeHtml(attachment.name || "Медиафайл");
  const size = formatAdminMailingFileSize(attachment.size);
  const dataUrl = attachment.dataUrl ? escapeAttribute(attachment.dataUrl) : "";
  let preview = `
    <div class="admin-mailing-attachment__placeholder" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path d="M4 7.5A2.5 2.5 0 0 1 6.5 5h11A2.5 2.5 0 0 1 20 7.5v9a2.5 2.5 0 0 1-2.5 2.5h-11A2.5 2.5 0 0 1 4 16.5z"/><path d="m7 15 2.8-3.1 2.2 2.4 1.7-1.8L17 16"/></svg>
    </div>`;

  if (dataUrl && attachment.kind === "image") {
    preview = `<img src="${dataUrl}" alt="${name}">`;
  } else if (dataUrl && attachment.kind === "video") {
    preview = `<video src="${dataUrl}" muted playsinline controls preload="metadata"></video>`;
  }

  return `
    <div class="admin-mailing-attachment">
      <div class="admin-mailing-attachment__preview">${preview}</div>
      <div class="admin-mailing-attachment__info">
        <strong>${label} прикреплено</strong>
        <span>${name} · ${size}</span>
      </div>
    </div>`;
}

function revokeAdminMailingSelectedMediaUrl() {
  if (adminMailingSelectedMediaUrl) URL.revokeObjectURL(adminMailingSelectedMediaUrl);
  adminMailingSelectedMediaUrl = "";
}

function renderAdminMailingSelectedMedia() {
  const preview = document.querySelector("#adminMailingMediaPreview");
  if (!preview) return;

  if (!adminMailingSelectedMedia) {
    preview.hidden = true;
    preview.innerHTML = "";
    return;
  }

  const { file, kind, url } = adminMailingSelectedMedia;
  const name = escapeHtml(file.name || "Медиафайл");
  const size = formatAdminMailingFileSize(file.size);
  const media = kind === "video"
    ? `<video src="${escapeAttribute(url)}" muted playsinline controls preload="metadata"></video>`
    : `<img src="${escapeAttribute(url)}" alt="${name}">`;

  preview.hidden = false;
  preview.innerHTML = `
    <div class="admin-mailing-media-preview__thumb">${media}</div>
    <div class="admin-mailing-media-preview__body">
      <strong>${kind === "video" ? "Видео прикреплено" : "Фото прикреплено"}</strong>
      <span>${name} · ${size}</span>
    </div>
    <button class="admin-mailing-media-preview__remove" type="button" data-admin-mailing-clear-media aria-label="Убрать вложение">
      <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 7 10 10M17 7 7 17"/></svg>
    </button>`;
}

function clearAdminMailingSelectedMedia() {
  revokeAdminMailingSelectedMediaUrl();
  adminMailingSelectedMedia = null;
  const input = document.querySelector("#adminMailingMediaInput");
  if (input) input.value = "";
  renderAdminMailingSelectedMedia();
}

function setAdminMailingMediaFile(file) {
  clearAdminMailingSelectedMedia();
  if (!file) return;

  const kind = getAdminMailingMediaKind(file);
  if (!kind) {
    showToast("Можно прикрепить только фото или видео");
    return;
  }

  const url = URL.createObjectURL(file);
  adminMailingSelectedMediaUrl = url;
  adminMailingSelectedMedia = { file, kind, url };
  renderAdminMailingSelectedMedia();
}

function readAdminMailingMediaDataUrl(file) {
  return new Promise((resolve) => {
    if (!file || file.size > adminMailingMediaPersistLimit) {
      resolve("");
      return;
    }
...
async function buildAdminMailingAttachment() {
  if (!adminMailingSelectedMedia?.file) return null;

  const { file, kind } = adminMailingSelectedMedia;
  return {
    name: file.name || "Медиафайл",
    type: file.type || "",
    size: file.size || 0,
    kind,
    dataUrl: await readAdminMailingMediaDataUrl(file)
  };
}

function adminMailingEmptyMarkup(hasQuery) {
  const title = hasQuery ? "Рассылка не найдена" : "Рассылок пока нет";
  const text = hasQuery
    ? "Попробуйте изменить запрос или выбрать другой статус."
    : "Создайте первую рассылку, чтобы отправлять клиентам акции, новости и напоминания.";
  const action = hasQuery
    ? ""
    : '<button class="admin-mailing-primary-button" type="button" data-admin-mailing-open-compose>Создать рассылку</button>';

  return `
    <article class="admin-mailing-empty card-surface">
      <div class="admin-mailing-empty__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24"><path d="M4 6.5h16v11H4z"/><path d="m4.5 7 7.5 6 7.5-6"/><path d="M8.5 20h7"/></svg>
      </div>
      <strong>${title}</strong>
      <p>${text}</p>
      ${action}
...
function renderAdminMailingStats(mailings) {
  const totalElement = document.querySelector("#adminMailingsTotal");
  const sentElement = document.querySelector("#adminMailingsSent");
  const draftsElement = document.querySelector("#adminMailingsDrafts");
  const sentMessages = mailings.reduce((sum, item) => sum + (item.status === "sent" ? Number(item.sentCount) || 0 : 0), 0);
  const draftCount = mailings.filter((item) => item.status === "draft").length;

  if (totalElement) totalElement.textContent = mailings.length;
  if (sentElement) sentElement.textContent = sentMessages;
  if (draftsElement) draftsElement.textContent = draftCount;
}

function renderAdminMailings() {
  const list = document.querySelector("#adminMailingList");
  if (!list) return;

  const input = document.querySelector("#adminMailingSearch");
  const query = String(input?.value || adminMailingSearch || "").trim().toLowerCase();
  const allMailings = getAdminMailings();
  renderAdminMailingStats(allMailings);

  const filtered = allMailings.filter((item) => {
    const matchesStatus = adminMailingFilter === "all" || item.status === adminMailingFilter;
    const haystack = `${item.title || ""} ${item.message || ""} ${item.audience || ""} ${item.channel || ""} ${item.attachment?.name || ""} ${adminMailingAttachmentLabel(item.attachment || {})}`.toLowerCase();
    const matchesSearch = !query || haystack.includes(query);
    return matchesStatus && matchesSearch;
  });

  if (!filtered.length) {
    list.innerHTML = adminMailingEmptyMarkup(Boolean(query));
    return;
  }

  list.innerHTML = filtered.map((item) => `
    <article class="admin-mailing-card card-surface">
      <div class="admin-mailing-card__top">
        <div>
          <span>${escapeHtml(item.channel || "Канал не выбран")}</span>
          <h2>${escapeHtml(item.title || "Без названия")}</h2>
        </div>
        <i class="admin-mailing-status ${adminMailingStatusClass(item.status)}">${escapeHtml(item.statusLabel || "Черновик")}</i>
      </div>
      <p>${escapeHtml(item.message || "Текст сообщения пока не заполнен.")}</p>
      ${adminMailingAttachmentMarkup(item.attachment)}
      <div class="admin-mailing-meta">
        <span>${escapeHtml(item.audience || "Получатели не выбраны")}</span>
        <span>${escapeHtml(item.date || "Дата не указана")}</span>
        <span>${Number(item.sentCount) || 0} отправлено</span>
        ${item.attachment ? `<span>${adminMailingAttachmentLabel(item.attachment)} вложено</span>` : ""}
      </div>
...
function openAdminMailingCompose() {
  const compose = document.querySelector("#adminMailingCompose");
  if (!compose) return;
  compose.hidden = false;
  document.querySelector("#adminMailingTitle")?.focus({ preventScroll: true });
  compose.scrollIntoView({ behavior: "smooth", block: "start" });
}

function closeAdminMailingCompose() {
  const compose = document.querySelector("#adminMailingCompose");
  if (compose) compose.hidden = true;
}

function resetAdminMailingForm() {
  ["#adminMailingTitle", "#adminMailingMessage"].forEach((selector) => {
    const field = document.querySelector(selector);
    if (field) field.value = "";
  });
  const audience = document.querySelector("#adminMailingAudience");
  if (audience) audience.value = "Все клиенты";
  clearAdminMailingSelectedMedia();
}
...
function setAdminPickupAddressLoading(loading) {
  const saveButton = document.querySelector("#adminPickupAddressSave");
  const resetButton = document.querySelector("#adminPickupAddressReset");
  const input = document.querySelector("#adminPickupAddress");
  if (saveButton) {
    saveButton.disabled = loading;
    saveButton.textContent = loading ? "Сохраняем…" : "Сохранить адрес";
  }
...
async function loadAdminSettings() {
  ...
  savedPickupAddress = pickupAddressValue();
  const input = document.querySelector("#adminPickupAddress");
  if (input) input.value = savedPickupAddress;
  updatePickupAddressPreview(savedPickupAddress);
  setAdminPickupAddressStatus("");
}

async function saveAdminPickupAddress() {
  const input = document.querySelector("#adminPickupAddress");
  const address = input?.value.trim() || "";

  if (!address) {
    setAdminPickupAddressStatus("Укажите адрес самовывоза.", "error");
    input?.focus();
    return;
  }
  ...
  const initData = getTelegramInitData();
  if (!initData) {
    setAdminPickupAddressStatus("Откройте админку внутри Telegram Mini App.", "error");
    showToast("Настройки сохраняются внутри Telegram Mini App", "error");
    return;
  }

  setAdminPickupAddressLoading(true);
  setAdminPickupAddressStatus("");
  try {
    const response = await fetch("/api/admin/settings/pickup-address", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "X-Telegram-Init-Data": initData
      },
      body: JSON.stringify({ init_data: initData, pickup_address: address })
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data.error || "Не удалось сохранить адрес");

    appSettings.pickupAddress = String(data.pickup_address || address).trim();
    savedPickupAddress = pickupAddressValue();
    if (input) input.value = savedPickupAddress;
    applyPickupAddressToCheckout();
    updateDeliveryFields();
    setAdminPickupAddressStatus("Адрес самовывоза сохранён.", "success");
    showToast("Адрес самовывоза сохранён");
    tg?.HapticFeedback?.notificationOccurred?.("success");
  } catch (error) {
    setAdminPickupAddressStatus(error.message || "Не удалось сохранить адрес.", "error");
    showToast(error.message || "Не удалось сохранить адрес", "error");
    tg?.HapticFeedback?.notificationOccurred?.("error");
  } finally {
    setAdminPickupAddressLoading(false);
  }
}

function resetAdminPickupAddressForm() {
  const input = document.querySelector("#adminPickupAddress");
  if (input) input.value = savedPickupAddress;
  updatePickupAddressPreview(savedPickupAddress);
  setAdminPickupAddressStatus("");
...
function buildAdminMailingRecord({
  title,
  message,
  audience,
  status,
  ...
  return {
    id: Date.now(),
    title: title || "Новая рассылка",
    message: message || "Текст сообщения пока не заполнен.",
    audience: audience || "Все клиенты",
    channel: "Telegram",
    status,
    statusLabel,
...
function setAdminMailingSendLoading(loading) {
  const button = document.querySelector("#adminMailingSendTelegram");
  if (!button) return;
  button.disabled = loading;
  button.textContent = loading ? "Отправляем…" : "Отправить в Telegram";
}

async function sendAdminMailingToTelegram() {
  const initData = getTelegramInitData();
  if (!initData) {
    showToast("Откройте админку внутри Telegram Mini App", "error");
    return;
  }

  const title = document.querySelector("#adminMailingTitle")?.value.trim() || "Новая рассылка";
  const message = document.querySelector("#adminMailingMessage")?.value.trim() || "";
  const audience = document.querySelector("#adminMailingAudience")?.value || "Все клиенты";
  const attachment = await buildAdminMailingAttachment();
  const file = adminMailingSelectedMedia?.file || null;

  if (!message && !file) {
    showToast("Добавьте текст, фото или видео", "error");
    return;
  }
  ...
  if (file) formData.append("media", file, file.name || "mailing-media");

  setAdminMailingSendLoading(true);
  try {
    const response = await fetch("/api/admin/mailings/send", {
      method: "POST",
      headers: {
        "X-Telegram-Init-Data": initData
      },
    ...
    const saved = readAdminMailings();
    const sentMailing = buildAdminMailingRecord({
      title,
      message,
      audience,
      status: "sent",
    ...
    try {
      saveAdminMailings([sentMailing, ...saved]);
    } catch {
      sentMailing.attachment = attachment ? { ...attachment, dataUrl: "" } : null;
      saveAdminMailings([sentMailing, ...saved]);
    }

    resetAdminMailingForm();
    closeAdminMailingCompose();
    renderAdminMailings();
    const failed = Number(data.failed_count) || 0;
    showToast(
      failed > 0
        ? `Отправлено: ${data.sent_count}, ошибок: ${failed}`
        : `Отправлено в Telegram: ${data.sent_count}`,
      "success"
    );
    tg?.HapticFeedback?.notificationOccurred?.("success");
  } catch (error) {
    showToast(error.message || "Не удалось отправить рассылку", "error");
    tg?.HapticFeedback?.notificationOccurred?.("error");
  } finally {
    setAdminMailingSendLoading(false);
  }
}

async function createAdminMailingDraft() {
  const title = document.querySelector("#adminMailingTitle")?.value.trim() || "Новая рассылка";
  const message = document.querySelector("#adminMailingMessage")?.value.trim() || "Текст сообщения пока не заполнен.";
  const audience = document.querySelector("#adminMailingAudience")?.value || "Все клиенты";
  ...
  const mailing = {
    ...
    audience,
    channel,
    status: timing === "Запланировать" ? "scheduled" : "draft",
    statusLabel: timing === "Запланировать" ? "Запланирована" : "Черновик",
    sentCount: 0,
    date: timing === "Запланировать" ? "Дата не выбрана" : "Сегодня",
    attachment
  };

  let notice = "Рассылка сохранена";
  try {
    saveAdminMailings([mailing, ...saved]);
  } catch {
    if (!mailing.attachment?.dataUrl) {
      showToast("Не удалось сохранить рассылку");
      return;
    }

    try {
      mailing.attachment.dataUrl = "";
      saveAdminMailings([mailing, ...saved]);
      notice = "Рассылка сохранена, медиа добавлено без превью";
    } catch {
      showToast("Не удалось сохранить рассылку");
      return;
    }
...
async function loadAdminOrders() {
  ...
  try {
    ...
    if (serverOrders.length) orders = serverOrders.map(order => ({
      ...order,
  ...
  const filtered = adminOrderFilter === "all"
    ? orders
    : orders.filter(order => (order.status || "new") === adminOrderFilter);

  list.innerHTML = filtered.map(order => `
    <article class="admin-order-card card-surface" data-admin-order-id="${order.id}">
      <div class="admin-order-card__top">
        <b>Заказ #${order.id}</b>
        <strong>${escapeHtml(order.label || order.status || "Новый")}</strong>
      </div>
      <div class="admin-order-card__content">
        <p><b>Клиент:</b> ${escapeHtml(order.customer_name || "Не указан")}</p>
        <p><b>Состав:</b> ${escapeHtml(order.items || "Нет данных")}</p>
        <p><b>Количество:</b> ${order.quantity || 1}</p>
        <p><b>Сумма:</b> ${money(order.total || 0)}</p>
        ${order.address ? `<p><b>Адрес:</b> ${escapeHtml(order.address)}</p>` : ""}
        ${order.comment ? `<p><b>Комментарий:</b> ${escapeHtml(order.comment)}</p>` : ""}
      </div>
    </article>`).join("") || `<article class="admin-order-card card-surface">Заказов нет</article>`;
}

document.addEventListener("click", (event) => {
  const button = event.target.closest("[data-admin-order-filter]");
  if (!button) return;
  document.querySelectorAll("[data-admin-order-filter]").forEach(item => item.classList.remove("is-active"));
  button.classList.add("is-active");
  adminOrderFilter = button.dataset.adminOrderFilter;
  loadAdminOrders();
});


document.addEventListener("click", (event) => {
  const card = event.target.closest("[data-admin-order-id]");
  if (card) {
    const order = demoOrders.find(item => String(item.id) === String(card.dataset.adminOrderId));
    if (order) openAdminOrder(order);
  }
  if (event.target.closest("#adminOrderClose")) {
    document.querySelector("#adminOrderOverlay").hidden = true;
  }
});
...
document.querySelector("#adminCustomersSearch")?.addEventListener("input", renderAdminCustomers);

document.querySelector("#adminPickupAddress")?.addEventListener("input", (event) => {
  updatePickupAddressPreview(event.target.value);
  setAdminPickupAddressStatus("");
});
document.querySelector("#adminPickupAddressSave")?.addEventListener("click", saveAdminPickupAddress);
document.querySelector("#adminPickupAddressReset")?.addEventListener("click", resetAdminPickupAddressForm);

document.addEventListener("click", (event) => {
  const button = event.target.closest("[data-admin-customers-clear]");
  if (!button) return;

  const input = document.querySelector("#adminCustomersSearch");
  if (input) input.value = "";
...
document.querySelector("#adminMailingSearch")?.addEventListener("input", (event) => {
  adminMailingSearch = event.target.value;
  renderAdminMailings();
});

["#adminCreateMailingButton", "[data-admin-mailing-open-compose]"].forEach((selector) => {
  document.addEventListener("click", (event) => {
    if (!event.target.closest(selector)) return;
    openAdminMailingCompose();
    tg?.HapticFeedback?.impactOccurred?.("light");
  });
});

document.querySelector("#adminMailingCloseCompose")?.addEventListener("click", () => {
  closeAdminMailingCompose();
});

document.querySelector("#adminMailingCancel")?.addEventListener("click", () => {
  closeAdminMailingCompose();
});

document.querySelector("#adminMailingSaveDraft")?.addEventListener("click", createAdminMailingDraft);

document.querySelector("#adminMailingSendTelegram")?.addEventListener("click", sendAdminMailingToTelegram);

document.querySelector("#adminMailingMediaInput")?.addEventListener("change", (event) => {
  setAdminMailingMediaFile(event.target.files?.[0] || null);
});

document.addEventListener("click", (event) => {
  if (!event.target.closest("[data-admin-mailing-clear-media]")) return;
  clearAdminMailingSelectedMedia();
  tg?.HapticFeedback?.impactOccurred?.("light");
});

document.addEventListener("click", (event) => {
  const button = event.target.closest("[data-admin-mailing-filter]");
  if (!button) return;

  document.querySelectorAll("[data-admin-mailing-filter]").forEach((item) => item.classList.remove("is-active"));
  button.classList.add("is-active");
  adminMailingFilter = button.dataset.adminMailingFilter || "all";
  renderAdminMailings();
  tg?.HapticFeedback?.selectionChanged?.();
});
  ...
  button.addEventListener("click", () => {
    if (button.dataset.adminSection === "customers") { setActivePage("admin-customers", {haptic:true}); renderAdminCustomers(); return; }
    if (button.dataset.adminSection === "notifications") { setActivePage("admin-mailings", {haptic:true}); renderAdminMailings(); return; }
    if (button.dataset.adminSection === "settings") { setActivePage("admin-settings", {haptic:true}); return; }
...
document.querySelector("#adminPickupAddressReset")?.addEventListener("click", resetAdminPickupAddressForm);
  ...
  button.addEventListener("click", () => {
    ...
    if (button.dataset.adminSection === "settings") setActivePage("admin-settings", {haptic:true});
  });
});
